...
def dashboard():
    ...
    total_prepared = sum(r.prepared_quantity for r in records)
    total_wasted = sum(r.wasted_quantity for r in records)

    total_loss = sum(
        r.wasted_quantity * r.cost_per_kg
        for r in records
    )

    if total_prepared > 0:
        waste_rate = (total_wasted / total_prepared) * 100
    else:
        waste_rate = 0
